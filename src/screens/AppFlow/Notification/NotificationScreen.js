...
import { OrganisationNotificationTable } from "../../../models";
...
const NotificationScreen = (props) => {
  ...
  console.log("NotificationScreen", props?.userId);
  ...
  useEffect(() => {
    const subscription = DataStore.observe(
      ...
      // if (msg.model === OrganisationNotificationTable && msg.opType === "DELETE") {
      //   getFacilityNotification(userId, lastDay);
      // }
    });
  ...
  const updateStatusNotification = async (data) => {
    const uJ = await DataStore.save(
      OrganisationNotificationTable.copyOf(data, (updated) => {
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        >
          ...
          <Text style={{ flex: 1, textAlign: "center" }}>No New Notification</Text>
        </View>
      ) : (
