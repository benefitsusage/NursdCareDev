...
import NoShowCard from "../../Manage/Card/NoShowCard";
...
const NoShowVisit = (props) => {
  const [userId, setUserId] = useState(
    props === undefined ? {} : props?.route?.params?.data?.id
  );
  // console.log("NoShowVisit", userId);
  ...
  const getFacilityJob = async (id, page) => {
    try {
      const itemArr = await DataStore.query(
        ...
        {
          sort: (s) => s.noShowReason(SortDirection.DESCENDING),
          page: page,
          limit: 10,
        }
      );
