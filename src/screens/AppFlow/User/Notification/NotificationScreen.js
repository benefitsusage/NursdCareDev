...
const NotificationScreen = (props) => {
  ...
  useEffect(() => {
    if (userId !== undefined) {
      getNurseNotification(userId, lastDay);
    }
  }, [userId, lastDay]);
  ...
  const getNurseNotification = async (id, day) => {
    setData(undefined);
  ...
  useEffect(() => {
    const subscription = DataStore.observe(NurseNotificationTable, (item) =>
      item.nurseTableID.eq(userId)
    ).subscribe((msg) => {
      if (msg.model === NurseNotificationTable && msg.opType === "INSERT") {
        getNurseNotification(userId, lastDay);
      }
      if (msg.model === NurseNotificationTable && msg.opType === "UPDATE") {
        getNurseNotification(userId, lastDay);
      }
      // if (msg.model === NurseNotificationTable && msg.opType === "DELETE") {
      //   getNurseNotification(userId, lastDay);
      // }
    });
  ...
  const deleteNotification = async () => {
    ...
    // After all deletes are done, call getNurseNotification
    getNurseNotification(userId, lastDay);
    setSelected(false);
    setSelectedCards([]);
  };
  ...
  //     NurseNotificationTable.copyOf(data, (updated) => {
